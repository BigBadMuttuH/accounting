...
namespace accounting.Model
{
    ...
    public class Device
    {
        ...
        public void GetDeviceInformFromSystem()
        {
            ...
            var usbDevices = new ManagementObjectSearcher(@"SELECT * FROM Win32_DiskDrive WHERE MediaType like ""Removable Media""").Get();
            if (usbDevices.Count > 0)
            {
                ...
                // Получем VID и PID
                string str = $"SELECT * FROM Win32_USBHub WHERE DeviceID like '%{SerialNumber}%' AND Status='OK'";
                ...
                foreach (var usbDevice in usbDevices)
                {
                    ...
                    {
                        Vid = deviceID.Substring(deviceID.IndexOf("VID_"), 8);
                    }
                    if (deviceID.Contains("PID_"))
                    {
                        Pid = deviceID.Substring(deviceID.IndexOf("PID_"), 8);
                    }
                }
            }
            else
